...
open class FSPageControl: UIControl {
    ...
    open override func layoutSublayers(of layer: CALayer) {
        ...
        if self.contentView.frame.width > self.contentView.frame.height {
            var x: CGFloat = {
            ...
            var y: CGFloat = {
                ...
                case .left, .leading:
                    return 0
                case .center, .fill:
                    let midY = self.contentView.bounds.midY
                    let amplitude = CGFloat(self.numberOfPages/2) * diameter + spacing*CGFloat((self.numberOfPages-1)/2)
                    return midY - amplitude
                case .right, .trailing:
                    let contentHeight = diameter*CGFloat(self.numberOfPages) + CGFloat(self.numberOfPages-1)*spacing
                    return contentView.frame.height - contentHeight
                }
            }()
            for (index,value) in self.indicatorLayers.enumerated() {
                let state: UIControl.State = (index == self.currentPage) ? .selected : .normal
                let image = self.images[state]
                let size = image?.size ?? CGSize(width: diameter, height: diameter)
                let origin = CGPoint(x: self.contentView.bounds.midX-size.width*0.5, y: y - (size.height-diameter)*0.5)
                value.frame = CGRect(origin: origin, size: size)
                y = y + spacing + diameter
            }
        }
    }
