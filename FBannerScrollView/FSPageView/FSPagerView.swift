...
open class FSPagerView: UIView,UICollectionViewDataSource,UICollectionViewDelegate {
    ...
    /// 控制轮播图的滚动方向 默认正向(向左和向上)
    @IBInspectable
    open var isPositive: Bool = false
    ...
    fileprivate func flipNext(sender: Timer?) {
        ...
        let contentOffset: CGPoint = {
            ...
            let tempIndexPathItem = indexPath.item+(self.isPositive ? 1 : -1)
            let section = self.numberOfSections > 1 ? (indexPath.section+tempIndexPathItem/self.numberOfItems) : 0
            let item = tempIndexPathItem % self.numberOfItems
