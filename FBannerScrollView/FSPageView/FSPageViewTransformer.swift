...
open class FSPagerViewTransformer: NSObject {
    ...
    open func applyTransform(to attributes: FSPagerViewLayoutAttributes) {
        ...
        case .coverFlow:
            if scrollDirection == .horizontal {
                ...
                var transform3D = CATransform3DIdentity
                transform3D.m34 = -0.002
                transform3D = CATransform3DRotate(transform3D, rotation, 1, 0, 0)
                transform3D = CATransform3DTranslate(transform3D, 0, translationZ, 0)
                attributes.zIndex = 100 - Int(abs(position))
                attributes.transform3D = transform3D
            }
        ...
        case .ferrisWheel, .invertedFerrisWheel:
            ...
            case -5 ... 5:
                ...
                transform = scrollDirection == .horizontal ? transform.translatedBy(x: -position*itemSpacing, y: ty) : transform.translatedBy(x: ty, y: -position*itemSpacing)
                transform = transform.rotated(by: rotation)
                transform = scrollDirection == .horizontal ? transform.translatedBy(x: 0, y: -ty) : transform.translatedBy(x: -ty, y: 0)
    ...
    open func proposedInteritemSpacing() -> CGFloat {
        ...
        switch self.type {
        case .overlap:
            return scrollDirection == .horizontal ? pagerView.itemSize.width * -self.minimumScale * 0.6:pagerView.itemSize.height * -self.minimumScale * 0.6
        case .linear:
            return scrollDirection == .horizontal ? pagerView.itemSize.width * -self.minimumScale * 0.2:pagerView.itemSize.height * -self.minimumScale * 0.2
        case .coverFlow:
            return scrollDirection == .horizontal ? -pagerView.itemSize.width * sin(.pi*0.25*0.25*3.0):-pagerView.itemSize.height * sin(.pi*0.25*0.25*3.0)
        case .ferrisWheel,.invertedFerrisWheel:
            return scrollDirection == .horizontal ? -pagerView.itemSize.width*0.15:-pagerView.itemSize.height * 0.15
        case .cubic:
            return 0
        default:
            break
